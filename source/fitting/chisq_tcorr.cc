...
RealMultiTemporalCorrelatorFit::RealMultiTemporalCorrelatorFit(
                  XMLHandler& xmlin, MCObsHandler& OH, int taskcount)   :  ChiSquare(OH)
{
 XMLHandler xmlf(xmlin,"TemporalCorrelatorFit");
 OperatorInfo COp(xmlf);
...
//    throw(std::invalid_argument("Must use Bootstrap mode with VEV subtractions"));
 ...
 xmlreadchild(xmlf,"MaximumTimeSeparation",tmax);
 if (tmin<0) throw(std::invalid_argument("Invalid MinimumTimeSeparation"));
 if (tmax<=tmin) throw(std::invalid_argument("Invalid MaximumTimeSeparation => Tmin="+to_string(tmin)+", Tmax="+to_string(tmax)));
 vector<int> texclude;
 string exstr;
 if (xmlreadifchild(xmlf,"ExcludeTimes",exstr)){
    extract_from_string(exstr,texclude);}
 m_tvalues_tot=form_tvalues(tmin,tmax,texclude);  // ascending order
 m_tvalues=form_tvalues(tmin,tmax,texclude);
   //  Reads and computes correlator estimates, returning estimates
   //  in the "results" map.  The integer key in this map is the
   //  time separation.  Note that results are also "put" into 
   //  the memory of the MObsHandler pointed to by "moh".
...
void RealMultiTemporalCorrelatorFit::evalGradients(
                               const vector<double>& fitparams,
                               RMatrix& gradients) const
{
 uint nparam=m_model_ptr->getNumberOfParams();
...
void RealMultiTemporalCorrelatorFit::guessInitialParamValues(
                               const RVector& datapoints,
                               vector<double>& fitparams) const
{
 vector<double> corr(datapoints.size());
...
void RealMultiTemporalCorrelatorFit::do_output(XMLHandler& xmlout) const
{
 xmlout.set_root("TemporalCorrelatorFit");
 XMLHandler xmlop;
 m_op.output(xmlop);
...
void RealMultiTemporalCorrelatorFit::pop_front_tlist(){
    m_tvalues.erase(m_tvalues.begin());
    m_obs_info.erase(m_obs_info.begin());
    m_nobs = m_nobs-1;
//     m_means.clear();
}
    
void RealMultiTemporalCorrelatorFit::reset_tlist( const vector<uint>& m_tvalues_tot, const vector<MCObsInfo>& m_obs_info_tot){ 
    m_tvalues = m_tvalues_tot; 
    m_obs_info = m_obs_info_tot; 

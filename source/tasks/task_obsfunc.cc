...
void TaskHandler::doObsFunction(XMLHandler& xmltask, XMLHandler& xmlout, int taskcount)
{
 ...
 /**************************************************************************************************************************************/
    ...
    // should this be restricted to two particle states?
    ...
    for (list<XMLHandler>::iterator it=denomxml.begin();it!=denomxml.end();++it){
       ...
       // it->getOptionalBool("SubtractVEV", subvev);
	...
	const RVector& bins1=m_obs->getBins(MCObsInfo(origcorr)); // manually subtract vevs here?
	RVector newbins(bins1);
	for (uint k=0;k<nterms;k++){
  ...
  /**************************************************************************************************************************************/
 ...
 else if (functype=="CorrelatorInteractingRatio2"){
    xmlout.set_root("DoObsFunction");
    xmlout.put_child("Type","CorrelatorInteractingRatio2");
    try{
    list<string> tagnames;
    tagnames.push_back("Operator");
    tagnames.push_back("OperatorString");
    ...
    pair<OperatorInfo,bool> numerator;
    bool subvev=(xmlint.count("SubtractVEV")>0) ? true: false;
    numerator=make_pair(OperatorInfo(xmlint),subvev);
    ...
    xmlreadchild(xmltask,"MinimumTimeSeparation",tmin);
    xmlreadchild(xmltask,"MaximumTimeSeparation",tmax);
    bool herm=true;		// digaonal corr elements only should appear
    string filename;
    bool writetofile=xmlreadifchild(xmltask,"WriteToBinFile",filename);
    if (filename.empty()) writetofile=false;
    bool overwrite = false;  // protect mode
    ...
    resultop.output(xmlo);
    xmlout.put_child(xmlo);
    xmlo.set_root("InteractingOperator"); // make sure this is correct
    ...
    uint count=0;
    set<MCObsInfo> obskeys;
    CorrelatorAtTimeInfo resultcorr(resultop,resultop,0,herm,numerator.second);
	...
	for (uint k=0;k<nterms;k++){
	  denomcorrs[k].resetTimeSeparation(t);}
	// getBins no good if there are vevs. How to deal with this? -- fit task instead?
	const RVector& bins1=m_obs->getBins(MCObsInfo(origcorr));
	RVector newbins(bins1);
	for (uint k=0;k<nterms;k++){
	  const RVector& binsk=m_obs->getBins(MCObsInfo(denomcorrs[k]));
	  for (uint abc=0;abc<binsk.size();abc++){
	    std::cout << k << "\t" << binsk[abc] << std::endl;}
	  RVector denombins(binsk);
	  newbins/=denombins;}
	m_obs->putBins(newkey,newbins);
	if (writetofile) obskeys.insert(newkey);
	...
	xmlout.put_child(xmlerr);}
       ...
       xmlout.clear();
       throw(std::invalid_argument(string("DoObsFunction with type CorrelatorInteractingRatio2 encountered an error: ")
             +string(errmsg.what())));} }

...
#include <stdexcept>
  ...
  //  Rotated operator:
  //
  //      icode[0] has the format (left to right)  
  //         14 bits level number, 15 bits uint key to static name string map,
  //           rightmost 3 bits set to 111 = 7 to indicate rotated
...
OperatorInfo::OperatorInfo(XMLHandler& xml_in)
{
 ...
 tags.insert("RotatedOperator");
 tags.insert("RotatedOpString");
 ...
 if (rtag=="OperatorString"){
    assign(xin.getString("OperatorString"));
    return;}

 if (rtag=="RotatedOperator"){
    xmlread_rotated(xin);
    return;}

 if (rtag=="RotatedOpString"){
    assign_rotated(xin.getString("RotatedOpString"));
    return;}
 }
    ...
    // converts specification of operator by a string into an OperatorInfo object
...
unsigned int OperatorInfo::getNumberOfHadrons() const   // returns 7 for rotated op
{
 return (icode[0] & nhad_mask);   // rightmost 3 bits
}

unsigned int OperatorInfo::get_NumberOfHadrons() const   // exception for rotated op
{
 uint nhad=(icode[0] & nhad_mask);   // rightmost 3 bits
 if (nhad==7) throw(std::invalid_argument("Rotated operator has no definite number of hadrons"));
 return nhad;
}

bool OperatorInfo::isVacuum() const
{
 return (getNumberOfHadrons()==0);
}

bool OperatorInfo::isGlueball() const
{
 return ((getNumberOfHadrons()==1)&&(isGlueball(1)));
}

bool OperatorInfo::isMeson() const
{
 return ((getNumberOfHadrons()==1)&&(isMeson(1)));
}

bool OperatorInfo::isBaryon() const
{
 return ((getNumberOfHadrons()==1)&&(isBaryon(1)));
}

bool OperatorInfo::isMesonMeson() const
{
 return ((getNumberOfHadrons()==2)&&(isMeson(1))&&(isMeson(2)));
}

bool OperatorInfo::isMesonBaryon() const
{
 return ((getNumberOfHadrons()==2)&&(isMeson(1))&&(isBaryon(2)));
}

bool OperatorInfo::isRotated() const
{
 return (getNumberOfHadrons()==7);   // notational only
}


Momentum OperatorInfo::getMomentum() const
{
...
int OperatorInfo::getXMomentum() const
{
 int p=0;
 for (unsigned int k=1;k<=get_NumberOfHadrons();k++) 
    p+=getXMomentum(k);
...
int OperatorInfo::getYMomentum() const
{
 int p=0;
 for (unsigned int k=1;k<=get_NumberOfHadrons();k++) 
    p+=getYMomentum(k);
...
int OperatorInfo::getZMomentum() const
{
 int p=0;
 for (unsigned int k=1;k<=get_NumberOfHadrons();k++) 
    p+=getZMomentum(k);
...
std::string OperatorInfo::getLGIrrep() const
{
...
unsigned int OperatorInfo::getLGClebschGordonIdNum() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
...
unsigned int OperatorInfo::getLGIrrepRow() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
...
std::string OperatorInfo::getIsospin() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
...
unsigned int OperatorInfo::getIsospinClebschGordonIdNum() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
 if (nhadrons<2) return 0;
...
std::string OperatorInfo::getFlavor() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
 if (nhadrons==0) return "singlet";
 else if (nhadrons==1) return get_flavor(icode[1]);
...
std::string OperatorInfo::getFlavorCode() const
{
 unsigned int nhadrons=get_NumberOfHadrons();
 if (nhadrons==0) return "0";
...
unsigned int OperatorInfo::get_rotated_obs_code() const
{
 return (icode[0]>>3)&0x7FFFu;
}

std::string OperatorInfo::getRotatedName() const
{
 if (!isRotated())
    throw(std::invalid_argument("Cannot get name since not rotated operator"));
 uint namecode=get_rotated_obs_code();
 string res;
 try{
    res=m_rotated_decodings[namecode].first;}
 catch(const std::exception& msg){
    throw(std::invalid_argument((string("Error in getRotatedName: ")
           +string(msg.what())).c_str()));}
 return res;
}

unsigned int OperatorInfo::getRotatedLevel() const
{
 if (!isRotated())
    throw(std::invalid_argument("Cannot get level since not rotated operator"));
 return icode[0]>>18;
}


OperatorInfo& OperatorInfo::resetRotatedLevel(uint level)
{
 if (!isRotated())
    throw(std::invalid_argument("Cannot reset level since not rotated operator"));
 uint bitmask=0x3FFFFu;
 icode[0]&=bitmask; // erase level
 icode[0]|=(level<<18);
 return *this;
}
...
std::string OperatorInfo::getFlavor(unsigned int hadron_index) const
{
 check_hadron_index(hadron_index,"getFlavor");
 return get_flavor(icode[2*hadron_index-1]);
}

int OperatorInfo::getStrangeness(unsigned int hadron_index) const
{
...
{
 bool notrotated=!isRotated();
 if (!longform){
    if (notrotated)
       xmlout.set_root("OperatorString",short_output());
    else
       xmlout.set_root("RotatedOpString",short_rotated_output());
    return;}
 if (notrotated){
    xmlout.set_root("Operator");
    ...
    xmlout.set_root("RotatedOperator");
    uint namecode=get_rotated_obs_code();
    xmlout.put_child("ObsName",m_rotated_decodings[namecode].first);
    xmlout.put_child("Level",make_string(getRotatedLevel()));
    const string& description=m_rotated_decodings[namecode].second;
    if (description.length()>0){
       XMLHandler xmld;
       xmld.set_from_string(description);
...
void OperatorInfo::check_hadron_index(unsigned int hadron_index, 
                                      const char *msg) const
{
 uint nhad=get_NumberOfHadrons();
 if ((hadron_index<1)||(hadron_index>nhad)){
    //cerr << "Invalid hadron index in OperatorInfo::"<<msg<<endl;
...
void OperatorInfo::xmlread_momentum(ArgsHandler& xmlh, const string& tag,
...
void OperatorInfo::xmlwrite_momentum(XMLHandler& xmlout, const string& tag,
...
void OperatorInfo::xmlread_hadron(ArgsHandler& xin, const string& toptag,
...
void OperatorInfo::xmlwrite_hadron(XMLHandler& xmlout, const string& toptag,
...
void OperatorInfo::xmlread_total(ArgsHandler& xin, unsigned int& code)
{
...
void OperatorInfo::xmlwrite_total(XMLHandler& xmlout, const unsigned int& code) const
{
 XMLHandler xmlt;
 xmlt.set_root("Total");
...
void OperatorInfo::xmlread_rotated(ArgsHandler& xin)
{
 string name(xin.getName("ObsName"));
 uint level=0;
 xin.getOptionalUInt("Level",level);
 string description;
 if (xin.queryTag("Description")){
    ArgsHandler ahd(xin,"Description");
    description=ahd.getInput();}
 encode_rotated(name,level,description);
}

void OperatorInfo::assign_rotated(const string& opstring)
{
 string opstr(tidyString(opstring));
 vector<string> tokens=split(opstr,' ');
 if ((tokens.size()!=2)&&(tokens.size()!=1)) 
    throw(std::invalid_argument("Invalid rotated operator string"));
 uint level=0;
 if (tokens.size()==2)
    try{extract_from_string(tokens[1],level);}
    catch(const std::exception& errmsg){
       throw(std::invalid_argument((string("Invalid RotatedOperatorInfo string: ")
                 +string(errmsg.what())).c_str()));}
 encode_rotated(tokens[0],level,string());
}


  // ***************************************************************

Momentum OperatorInfo::get_momentum(unsigned int momcode) const
{   
...
int OperatorInfo::get_xmomentum(unsigned int momcode) const
{
...
int OperatorInfo::get_ymomentum(unsigned int momcode) const
{
...
int OperatorInfo::get_zmomentum(unsigned int momcode) const
{
 unsigned int tmp=(momcode>>nhad_bits);
 int res=tmp & momj_mask;
 if (((tmp>>momj_bits)&0x1u)==1) return -res;
...
std::string OperatorInfo::get_flavor(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return m_flavor.decode(fcode);
}

std::string OperatorInfo::get_flavorcode(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return m_flavorcode.decode(fcode);
}

bool OperatorInfo::is_glueball(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return (fcode==1);
}

bool OperatorInfo::is_meson(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return ((fcode>=2)&&(fcode<=6));
}

bool OperatorInfo::is_baryon(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return ((fcode>=7)&&(fcode<=12));
}

bool OperatorInfo::is_fermion(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return ((fcode>=7)&&(fcode<=12));
}

bool OperatorInfo::is_boson(unsigned int hadroncode) const
{
 unsigned int fcode=hadroncode & flav_mask;
 return (fcode==1) || ((fcode>=2)&&(fcode<=6));
}

std::string OperatorInfo::get_lgirrep(unsigned int hadroncode) const
{
 unsigned int irrepcode=(hadroncode>>flav_bits) & irrp_mask;
 return m_irreps.decode(irrepcode);
}

std::string OperatorInfo::get_spatial_type(unsigned int hadroncode) const
{
 unsigned int spcode=(hadroncode>>(irrp_bits+flav_bits)) & sptp_mask;
 return m_spatial.decode(spcode);
}

unsigned int OperatorInfo::get_spatial_id(unsigned int hadroncode) const
{
 return (hadroncode>>(sptp_bits+irrp_bits+flav_bits)) & spid_mask;
}

unsigned int OperatorInfo::get_disp_length(unsigned int hadroncode) const
{
...
OperatorInfo::Encoder OperatorInfo::m_flavor(0);
OperatorInfo::Encoder OperatorInfo::m_irreps(1);
OperatorInfo::Encoder OperatorInfo::m_isospin(2);
OperatorInfo::Encoder OperatorInfo::m_spatial(3);
OperatorInfo::Encoder OperatorInfo::m_flavorcode(4);
OperatorInfo::Encoder OperatorInfo::m_isochar(5);


OperatorInfo::Encoder::Encoder(int ctype)
{
...
void OperatorInfo::Encoder::set_flavor()
{
 m_codetype="flavor";
...
void OperatorInfo::Encoder::set_flavorcode()
{
 m_codetype="FC";
     // zero quark operators
...
void OperatorInfo::Encoder::set_spatial()
{
...
void OperatorInfo::Encoder::set_irreps()
{
...
void OperatorInfo::Encoder::set_isospin()
{
...
void OperatorInfo::Encoder::set_isospin_char()
{
...
unsigned int OperatorInfo::Encoder::encode(const std::string& description) const
{
 map<string,unsigned int>::const_iterator it=m_code.find(description);
 if (it==m_code.end()) 
    throw(std::invalid_argument((string("Invalid ")+m_codetype
                  +string(" string in OperatorInfo")).c_str()));
 return it->second;
}
 
std::string OperatorInfo::Encoder::decode(unsigned int code) const
{
 map<unsigned int,string>::const_iterator it=m_string.find(code);
 if (it==m_string.end()) 
    throw(std::invalid_argument((string("Invalid ")+m_codetype
             +string(" code in OperatorInfo")).c_str()));
 return it->second;
}


int OperatorInfo::count(const string& astr, char delimiter) const
{
 int cnt=0;
...
vector<string> OperatorInfo::split(const string& astr, char delimiter) const
{
...
string OperatorInfo::extract(const string& astr, char left, char right) const
{
...
void OperatorInfo::assign(const std::string& opstring)
{
 try{
 string opstr(tidyString(opstring));
...
void OperatorInfo::encode_momentum(const std::string& momstr, unsigned int& momcode)
{
...
void OperatorInfo::encode_hadron(const std::string& flav, const std::string& irrep,
...
void OperatorInfo::encode_total(const std::string& totalisospin, const std::string& isocgid,
...
string OperatorInfo::short_output() const
{
 ...
 else if (nhadrons==7){ // rotated
    return short_rotated_output();}
 else{
...
void OperatorInfo::shortwrite_hadron(unsigned int momcode, unsigned int hadroncode,
...
void OperatorInfo::shortwrite_total(unsigned int code, string& totaliso, string& totirrep) const
{
 unsigned int irreprow=code;
 unsigned int lgcgid=irreprow>>irrw_bits;
 unsigned int ircode=lgcgid>>lgcg_bits;
...
string OperatorInfo::short_rotated_output() const
{
 return getRotatedName()+string(" ")+make_string(getRotatedLevel());
}
...
void OperatorInfo::encode_rotated(const string& inname, uint level, 
                                  const string& description)
{
 string name(tidyName(inname));
 if (name.length()>32){
    throw(std::invalid_argument("RotatedOperator name too long"));}
 if (level>=8192){
    throw(std::invalid_argument("RotatedOperator index too large"));}
 uint namecode=0;
 map<string,uint>::iterator mt=m_rotated_encodings.find(name);
 if (mt!=m_rotated_encodings.end()) namecode=mt->second;
 else{
    namecode=m_rotated_encodings.size();
    if (namecode>=65536){
       throw(std::invalid_argument("OperatorInfo too many names"));}
    m_rotated_encodings.insert(make_pair(name,namecode));
    m_rotated_decodings.push_back(make_pair(name,description));}
 uint tcode=level;
 tcode<<=15;
 tcode|=namecode;
 tcode<<=3;
...
std::map<std::string,uint> OperatorInfo::m_rotated_encodings;
std::vector<std::pair<std::string,std::string> > OperatorInfo::m_rotated_decodings;

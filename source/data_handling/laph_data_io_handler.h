...
namespace LaphEnv {
 ...
 // *                                                                               *
 // *         LapHDataGetHandlerMF   (multi-file)                                   *
 // *                                                                               *
 ...
 // *   the data from files and do NOT store the results in memory.  This class     *
 // *   is templated on a file key type, and record key type, and a data type:      *
 // *                                                                               *
 ...
 // *                                                                               *
 // *      LapHDataGetHandlerMF<FileKey,RecordKey,DataType>(                        *
...
class LapHDataGetHandlerMF
{
 ...
 public:
    ...
    LapHDataGetHandlerMF(const std::vector<std::string>& stubs,
                         const std::map<F,std::pair<int,int> >& data_files,
                         const std::string& filetype_id, 
                         unsigned int in_max_open_files,
                         double cleanfrac=0.25,
 ...
 private:
    ...
    void fail(const std::string& msg);
          ...
          // disallow copies
    LapHDataGetHandlerMF(const LapHDataGetHandlerMF& in);
    LapHDataGetHandlerMF& operator=(const LapHDataGetHandlerMF& in);
...
template <typename F, typename R, typename D>
LapHDataGetHandlerMF<F,R,D>::LapHDataGetHandlerMF(
                     const std::vector<std::string>& stubs,
              ...
              defaultclean(getNumberToClose(cleanfrac,in_max_open_files))
{
 if ((maxopenfiles<4)||(maxopenfiles>8192))
    throw(std::invalid_argument("Max open files in LapHDataGetHandlerMF is unreasonable"));
 for (typename std::map<F,std::pair<int,int> >::const_iterator 
    ...
    if ((stubind<0)||(stubind>=int(stubs.size())))
       throw(std::invalid_argument("Improper stub index in LapHDataGetHandlerMF"));
...
template <typename F, typename R, typename D>
bool LapHDataGetHandlerMF<F,R,D>::queryData(const F& fkey, const R& rkey)
{
...
template <typename F, typename R, typename D>
bool LapHDataGetHandlerMF<F,R,D>::queryFile(const F& fkey)
{
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::getData(const F& fkey, const R& rkey,
                                          D& result)
{
 IOMap<R,D> *fptr=get_file_ptr(fkey);
 if (fptr==0) fail(fkey);
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::close(const F& fkey)
{
 typename FileMapType::iterator it=fileMap.find(fkey);
 if (it!=fileMap.end()){
    close(it->second);}
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::close(double fraction)
{
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::close()
{
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::getFileMap(XMLHandler& xmlout) const
{
...
template <typename F, typename R, typename D>
std::set<F> LapHDataGetHandlerMF<F,R,D>::getFileKeys() const
{
 std::set<F> filekeys;
 for (typename FileMapType::const_iterator it=fileMap.begin();
      it!=fileMap.end();it++)
...
template <typename F, typename R, typename D>
std::set<R> LapHDataGetHandlerMF<F,R,D>::getKeys(const F& fkey)
{
 std::set<R> keys;
 IOMap<R,D>* fptr=get_file_ptr(fkey);
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::outputKeys(XMLHandler& xmlout)
{
...
template <typename F, typename R, typename D>
IOMap<R,D>* LapHDataGetHandlerMF<F,R,D>::get_file_ptr(const F& fkey)
{
 typename FileMapType::iterator it=fileMap.find(fkey);
 if (it==fileMap.end()) return 0;
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::open(FileMapValue& fmv)
{
 ...
 try {
    ...
    fail("failure opening file "+getFileName(fmv)+" in LapHDataGetHandlerMF");}
}


template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::close(FileMapValue& fmv)
{
 delete fmv.fptr; 
 fmv.fptr=0;
 for (typename std::list<LapHDataGetHandlerMF<F,R,D>::FileMapValue* >::iterator 
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::close(unsigned int number_to_close)
{
...
template <typename F, typename R, typename D>
std::string LapHDataGetHandlerMF<F,R,D>::getFileName(const FileMapValue& fmv) const
{
 std::stringstream fs;
...
template <typename F, typename R, typename D>
unsigned int LapHDataGetHandlerMF<F,R,D>::getNumberToClose(
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::fail(const F& fkey, const R& rkey)
{
 XMLHandler xmlout;
...
template <typename F, typename R, typename D>
void LapHDataGetHandlerMF<F,R,D>::fail(const F& fkey)
{
 XMLHandler xmlout;
 fkey.output(xmlout);
